...
class VehicleGeometry():
	...
	def getNewPoints(self, x, y, z, yaw, pitch, roll):
		...
		#  Generate DCM from Euler angles
		DCM = Rotations.euler2DCM(yaw,pitch,roll)

		# Based on Linear  Algebra tricks that Gabe has imparted to me
		# v_i ^T = (DCM^T*V_b)^T => v_b*DCM
		inertial_vertices = MatrixMath.matrixMultiply(self.vertices,DCM)

		# alternative to matrixOffset for translation for troubleshooting...
		# add as 16x3 matrix of the displacement to each and every point in space.
		# newPoints = MatrixMath.matrixAdd(inertial_vertices, [[x,y,z] for _ in inertial_vertices])

		# Translate converted vertices using matrixOffset
		newPoints = MatrixMath.matrixOffset(inertial_vertices,x,y,z)
